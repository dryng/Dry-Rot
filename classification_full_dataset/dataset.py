...
class DryRotDataset(Dataset):
    ...
    """
    def __init__(self, dset=0, path='/work/dryngler/dry_rot/datasets/classification_dataset_v2.h5', transform=None):
        self.dset = dset
        self.path = path
    ...
    def __getitem__(self, idx):
        with h5py.File(self.path) as f:
            if self.dset == 0:
                ...
                target = np.array(f["Y_train"][idx], dtype='f')
            elif self.dset == 1:
                image = np.array(f["X_val"][idx])
                target = np.array(f["Y_val"][idx], dtype='f')
            elif self.dset == 2:
                image = np.array(f["X_test"][idx])
                target = np.array(f["Y_test"][idx], dtype='f')
