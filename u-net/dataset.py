...
class DryRotDataset(Dataset):
    ...
    """
    def __init__(self, dset=0, path='/work/dryngler/dry_rot/datasets/segmentation_dataset.h5', transform=None):
        self.dset = dset
        self.path = path
    ...
    def __getitem__(self, idx):
        with h5py.File(self.path) as f:
            if self.dset == 0:
                ...
                mask = np.array(f["Y_train"][idx], dtype='f')
            elif self.dset == 1:
                image = np.array(f["X_val"][idx])
                mask = np.array(f["Y_val"][idx], dtype='f')
            elif self.dset == 2:
                image = np.array(f["X_test"][idx])
                mask = np.array(f["Y_test"][idx], dtype='f')
